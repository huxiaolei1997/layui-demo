    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
         // console.log(res);
          var resRes = response.resRes ? res[response.resRes] : res;
          var reaData = response.resData ? res[response.resData] : res;
          if(resRes[response.statusName] != response.statusCode){
            that.renderForm();
            that.layMain.html('<div class="'+ NONE +'">'+ (res[response.msgName] || '返回的数据状态异常') +'</div>');
          } else {
            that.renderData(reaData, curr, reaData[response.countName]), sort();
            options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
          }
          loadIndex && layer.close(loadIndex);
          typeof options.done === 'function' && options.done(reaData, curr, reaData[response.countName]);
        }
        ,error: function(e, m){
